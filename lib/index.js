...
const defaults = require('@nuxtjs/auth/lib/module/defaults');

const libRoot = resolve(require.resolve('@nuxtjs/auth'), '../../');

function validateOptions(options) {
    if (options.endpoints) {
        // eslint-disable-next-line no-console
        console.error('`auth.endpoints` has been moved into `auth.strategies.local`.');
    ...
    if (options.vuex && !this.options.store) {
        console.error('Enable vuex store by creating `store/index.js`.');
    }
}
...
module.exports = function(moduleOptions) {
    ...
    const options = merge.all([defaults, moduleOptions, this.options.auth]);
    ...
    if (!options.defaultStrategy) {
        ...
        console.warn(`strategy ${ options.defaultStrategy } is not defined!`);
    }
